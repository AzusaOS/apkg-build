...
func (e *buildEnv) orgMoveLib() error {
	...
	for _, p := range e.backend.FindFiles(e.dist, "*.la") {
		log.Printf("remove: $D/%s", p)
		e.backend.Remove(filepath.Join(e.dist, p))
	}

	for _, sub := range []string{"lib", "lib32", "lib64"} {
		st, err := e.backend.Lstat(filepath.Join(e.dist, e.getDir("core"), sub))
		if err != nil {
			continue
		}
...
func (e *buildEnv) orgFixMultilib() error {
	...
	for _, typ := range []string{"core", "libs", "dev"} {
		...
		_, errs := e.backend.Lstat(filepath.Join(e.dist, e.getDir(typ), "lib"))
		if errs != nil {
			continue
		}
		_, errd := e.backend.Lstat(filepath.Join(e.dist, e.getDir(typ), "lib"+e.libsuffix))
		if errd != nil {
			// source exists but not dest, move it
			log.Printf("rename %s to %s", filepath.Join(e.getDir(typ), "lib"), "lib"+e.libsuffix)
			e.backend.Rename(filepath.Join(e.dist, e.getDir(typ), "lib"), filepath.Join(e.dist, e.getDir(typ), "lib"+e.libsuffix))
			e.backend.Symlink("lib"+e.libsuffix, filepath.Join(e.dist, e.getDir(typ), "lib"))
		}
	}
	return nil
}
...
func (e *buildEnv) orgMoveEtc() error {
	if _, err := e.backend.Stat(filepath.Join(e.dist, "etc")); err == nil {
		e.backend.MkdirAll(filepath.Join(e.dist, e.getDir("core")), 0755)
		return e.backend.Rename(filepath.Join(e.dist, "etc"), filepath.Join(e.dist, e.getDir("core"), "etc"))
	}
	return nil
}
...
func (e *buildEnv) orgFixDev() error {
	...
	for _, sub := range []string{"pkgconfig", "cmake"} {
		if st, err := e.backend.Stat(filepath.Join(e.dist, e.getDir("libs"), "lib"+e.libsuffix, sub)); err == nil && st.IsDir() {
		...
		if st, err := e.backend.Stat(filepath.Join(e.dist, e.getDir("core"), "share", sub)); err == nil && st.IsDir() {
			// this should be moved to dev
	...
	if st, err := e.backend.Stat(filepath.Join(e.dist, e.getDir("core"), "include")); err == nil && st.IsDir() {
	...
	if st, err := e.backend.Stat(filepath.Join(e.dist, e.getDir("libs"), "lib"+e.libsuffix)); err == nil && st.IsDir() {
		// locate any .a files
		list := e.backend.FindFiles(filepath.Join(e.dist, e.getDir("libs"), "lib"+e.libsuffix), "*.a")
		if len(list) > 0 {
			e.backend.MkdirAll(filepath.Join(e.dist, e.getDir("dev"), "lib"+e.libsuffix), 0755)
			for _, f := range list {
	...
	list := e.backend.FindFiles(filepath.Join(e.dist, e.getDir("dev")), "*.a")
	for _, f := range list {
...
func (e *buildEnv) orgFixDoc() error {
	for _, sub := range []string{"man", "info"} {
		if st, err := e.backend.Stat(filepath.Join(e.dist, e.getDir("core"), sub)); err == nil && st.IsDir() {
...
func (e *buildEnv) orgFixUdev() error {
	if _, err := e.backend.Stat(filepath.Join(e.dist, "lib", "udev")); err == nil {
		// badly installed udev rules
		e.backend.MkdirAll(filepath.Join(e.dist, e.getDir("core")), 0755)
		err = e.backend.Rename(filepath.Join(e.dist, "lib", "udev"), filepath.Join(e.dist, e.getDir("core"), "udev"))
		if err != nil {
			return err
		}
		e.backend.Remove(filepath.Join(e.dist, "lib"))
	}

	if st, err := e.backend.Stat(filepath.Join(e.dist, e.getDir("libs"), "lib"+e.libsuffix, "udev")); err == nil && st.IsDir() {
...
func (e *buildEnv) orgFixFonts() error {
	...
	sublist, err := e.backend.ReadDir(filepath.Join(e.dist, "pkg", "main"))
	if err != nil {
		return err
	}
	for _, pkgInfo := range sublist {
		...
		if strings.HasPrefix(pkg, "media-fonts.font-util.core") {
			if st, err := e.backend.Stat(filepath.Join(e.dist, "pkg", "main", pkg, "share", "fonts")); err == nil && st.IsDir() {
				// this needs moving
				e.backend.MkdirAll(filepath.Join(e.dist, e.getDir("fonts")), 0755)
				entries, err := e.backend.ReadDir(filepath.Join(e.dist, "pkg", "main", pkg, "share", "fonts"))
				if err != nil {
					return err
				}
				for _, entry := range entries {
					ename := entry.Name()
					err = e.backend.Rename(filepath.Join(e.dist, "pkg", "main", pkg, "share", "fonts", ename), filepath.Join(e.dist, e.getDir("fonts"), ename))
					if err != nil {
						return err
					}
				}
...
func (e *buildEnv) orgFixPython() error {
	...
	sublist, err := e.backend.ReadDir(filepath.Join(e.dist, e.getDir("libs"), "lib"+e.libsuffix))
	if err != nil {
	...
	for _, subinfo := range sublist {
		...
		pyVer, err := e.backend.Readlink("/pkg/main/dev-lang.python.core." + pyVerPrefix) // dev-lang.python.core.3.10.2.linux.amd64
		if err != nil {
			return err
		}
		pyVer = strings.TrimPrefix(pyVer, "dev-lang.python.core.") // 3.10.2.linux.amd64
		pyVer = trimOsArch(pyVer)
		e.backend.MkdirAll(filepath.Join(e.dist, e.getDir("mod")+".py"+pyVer, "lib"), 0755)
...
func (e *buildEnv) moveAndLinkDir(src, dst string) error {
	...
	if _, err := e.backend.Stat(filepath.Join(e.dist, dst)); err != nil {
		err = e.backend.MkdirAll(filepath.Join(e.dist, dst), 0755)
		if err != nil {
			return err
		}
	}
	list, err := e.backend.ReadDir(filepath.Join(e.dist, src))
	if err != nil {
		return err
	}
	for _, fi := range list {
		nam := fi.Name()
		err = e.backend.Rename(filepath.Join(e.dist, src, nam), filepath.Join(e.dist, dst, nam))
		if err != nil {
			return err
		}
	}
	// remove dir
	err = e.backend.Remove(filepath.Join(e.dist, src))
	if err != nil {
		return err
	}
	// symlink to dst
	return e.backend.Symlink(dst, filepath.Join(e.dist, src))
}
...
func (e *buildEnv) moveAndLinkFile(src, dst string) error {
	...
	// src & dst start with "/pkg/main" - need to prepend e.dist if using
	if _, err := e.backend.Stat(filepath.Join(e.dist, dstdir)); err != nil {
		err = e.backend.MkdirAll(filepath.Join(e.dist, dstdir), 0755)
		if err != nil {
			return err
		}
	}
	err := e.backend.Rename(filepath.Join(e.dist, src), filepath.Join(e.dist, dst))
	if err != nil {
		return err
	}
	// symlink to dst
